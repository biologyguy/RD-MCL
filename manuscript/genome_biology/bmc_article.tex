...
\subsection{Wnt proteins as a benchmark dataset}\label{subsec:benchmarkDatasetWnts}
The `wingless-related integration site' (Wnt) proteins comprise a gene family with \textasciitilde{}20 members that are remarkably well conserved in vertebrates.
...
If an orthogroup is explicitly defined as ``the set of genes that are descended from a single gene in the last common ancestor of all the species being considered"~\ref{Emms:2015ig}, then we can devise a scoring system to assess the relative quality of any putative orthogroups based solely on the number of genes from each organism in a cluster and the overall distribution of genes across all clusters (i.e.,\ independent of sequence alignment or phylogenetic inference).
The details of this `orthogroup score' are outlined in the Materials and Methods~\ref{subsec:rd-mclFitnessFunction} subsection and Figure~\ref{fig:orthogroupscores}, but, intuitively, a cluster with many sequences each from distinct organisms will be scored higher than a cluster of the same size with many sequences from the same organism.
Furthermore, the relative influence a given species can have on the score should be scaled according to how many paralogs that species possesses, therefore preventing species-specific gene expansion from confounding the overall picture of homology within the family.
...
\subsection{Choosing an appropriate sequence similarity metric for resolving orthogroups from a single gene family}\label{subsec:increasingMclResolutionWithBlastp-freeSequenceSimilarityMetric}

%% Only show figures

Historically, the negative common log (-log\textsubscript{10}) of the BLASTP expectation value (NLE) has been used as the edge-weighting metric for graph-based ortholog prediction~\cite{Enright:2002uq,Li:2003en}.



Similarity metrics derived from BLASTP bit scores or expectation values (E-value) are the primary edge-weight values used for homology inference by MCL~\cite{Gibbons:2015be}.


The transition (stochastic) matrix used by MCL requires that the similarity metric is scaled between 0 and 1.
This reduces the applicability of unbounded scoring functions, such as raw bit-scores, because the distribution of similarity scores can be arbitrarily skewed by outliers.
The primary issue with the bit-score is a strong length bias, but this can be corrected for by scaling directly against alignment length or by scaling against a linear model derived from the best matches~\cite{Emms:2015ig}.

BLASTP scores reduce the overall resolving power of MCL when sequences are too similar or too dissimilar.
...
Local maxima in cluster number and orthogroup scores are possible across MCL parameter space (see all RefSeq caspase RD-MCL scores).

This means that a simple optimization algorithm is not appropriate, hence the choice to use Metropolis-Hastings.
