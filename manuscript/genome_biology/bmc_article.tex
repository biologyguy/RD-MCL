...
\startlocaldefs
\endlocaldefs
\newcommand{\goodtilde}{\raisebox{-0.7ex}{\textasciitilde}}
...
\subsection{Recursive MCL compensates for variable evolutionary separation among orthogroups}\label{subsec:recursiveMcl}
The mammalian and avian WNT orthogroups from the previous section contain very few duplicate taxa in any given cluster(\textgreater{}5\%), meaning the proteins separated cleaning into groups of one-to-one orthologs.
When WNTs from teleost fish are considered, however, the picture becomes less clear.
About 250\textendash{}350 million years ago the ancestral lineage of teleostean fishes experiences a whole genome duplication event, followed by a complicated pattern of gene loss, subfunctionalization, and neofunctionalization~\cite{Jaillon2004}.
Several of the duplicated WNTs were maintained as novel paralogs~\cite(Garriock:2007hb), meaning the `age' of these orthogroups is considerably younger than the canonical compliment of chordate WNTs, many of which originated before the radiation of ancestral bilaterians some 800 million years ago~\cite{Kusserow:2005js}.
Even after optimizing MCL parameters, there are several teleost WNT orthogroups with as many as 40\% duplicate taxa.
This is caused by the relatively small amount of sequence differentiation among these teleost-specific orthogroups relative to the large differentiation among the older orthogroups.
These differences lead to a bimodal distribution of incompatible `orthogroup thresholds' in MCL parameter space, where parameters that are good at separating old orthogroups cause the young orthogroups to merge, while parameters that can differentiate young orthogroups also erroneously break apart older orthogroups.
To address this challenge, the orthogroups scoring function is kept relatively permissive to replicate taxa in a cluster, then each cluster is recursively subdivided by further rounds of optimized MCL\@.
Applying recursion in this manner increases the best orthogroups score for the teleost WNTs by 7.3\%, to \goodtilde92.6\% of the theoretical maximum (Figure~\ref{fig:recursion_improvement}A).
The primary impact on the actual clusters is the separation of Wnt-2b, Wnt-3/3a, Wnt7a, and Wnt7b each into two distinct orthogroups (Figure~\ref{fig:recursion_improvement}B\textendash{}C).
When the sequences from these clusters were used to infer a maximum likelihood phylogeny, they assorted into clades that matched almost perfectly with those predicted by recursive MCL (Figure~\ref{fig:recursion_improvement}D).
Therefore, while a single round of MCL may be unable to separate orthogroups that have experienced dramatically different levels of sequence evolution, the issue can be resolved by recursively re-optimizing MCL parameters for each cluster until the best orthogroups score is identified, thus allowing a more nuanced separation of sequences.
To encompass the dynamic optimization of MCL parameters and recursive search strategy, we have named this new method and the accompanying software package \textit{R}ecursive \textit{D}ynamic \textit{M}arkov \textit{CL}ustering (RD-MCL).

\begin{figure*}[t]
  \begin{center}
  \includegraphics[height=0.8\textheight]{../figures/recursion_improvement.eps}
\end{center}
