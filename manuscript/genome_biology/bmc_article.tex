...
Unfortunately, this does not resolve another issue arising from the \textit{local} nature of BLAST alignment, where very dissimilar sequences that fall below detection threshold along most of their length are assigned high similarity scores because of a short conserved motif (Figure~\ref{fig:Bitscores}D).
Again, further fixes are feasible, but we have chosen to discard BLASTP-based similarity metrics in favour of a multiple sequence alignment-based method in the current implementation of RD-MCL\@.
