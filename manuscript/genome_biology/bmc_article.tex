...
This proportion will become higher when more diverse protein families or greater taxonomic coverage are considered, and while these edges may still be pruned away to optimize orthogroup assignment by MCL, we cannot know this beforehand.
...
It is, however, perfectly feasible to include an arbitrary number of sequence properties into the similarity metric, so we have devised a new scoring method that forces global alignment of all sequences and considers a substitution matrix along with predicted secondary structure properties.
Figure~\ref{fig:rdmclscores} illustrates that this new scoring function is independent of sequence length and, while correlated with the Emms transform scores, presents a unique dynamic range.
The details of the RD-MCL scoring function are elaborated in Materials and Methods section~\ref{subsec:seq-sim-func} and is further compared against BLAST-based metrics for orthogroup assignment in section~\ref{subsec:TuningParams}.
