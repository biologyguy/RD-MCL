...
This leaves a gap in our ability to easily discern evolutionary patterns at this scale and has exacerbated the propagation of annotation errors in our public databases~\cite{Schnoes:2009gb}, which still rely heavily on annotation transfer following inference of homology against a limited number of reference species~\cite{Aken:2016dl,Mi:2016bw,OLeary:2016cm}.
...
\subsection{Resolution increases when the sequence similarity metric is improved}\label{subsec:overcomingBlast-basedSimilarityMetricLimitations}
BLAST-based similarity metrics (bit score or e-value) have a strong length bias when used to identify orthogroups, but this can be corrected for by creating a linear model from the best matches, then scaling the entire pairwise similarity graph with that model~\cite{Emms:2015ig}.
Unfortunately,
