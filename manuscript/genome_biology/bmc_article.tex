...
The `wingless-related integration site' (Wnt) proteins comprise a gene family with approximately 20 members that are remarkably well conserved in vertebrates.
...
Primary isoforms for each Wnt gene belonging to species in \textit{Mammalia}, \textit{Aves}, or \textit{Actinopterygii} were identified in the RefSeq database using PSI-BLAST, and only minor filtering was applied to remove sequences that clearly fall outside of the normal complement of orthogroups (e.g.,\ sequences smaller than 250 or larger than 650 residues, and several duplicate entries for \textit{Home sapiens}, \textit{Mus musculus}, \textit{Rattus norvegicus}, and \textit{Bos taurus}).
...
\subsection{Defining an orthogroup score}\label{subsec:definingAnOrthogroupScore}
...
The details of this `orthogroup score' are outlined in subsection~\ref{subsec:rd-mclFitnessFunction} of the Materials and Methods and Figure~\ref{fig:orthogroupscores}, but, intuitively, a cluster with many sequences each from distinct organisms will be scored higher than a cluster of the same size with many sequences from the same organism.
...
While these scores allow us to compare putative orthogroups within a given dataset, they are relative and unitless, so they should not be used to directly compare orthogroups from different datasets.
...
The upper left panel represents all 1,367,031 comparisons for the 1,654 sequences included in this study, while the remaining panels are restricted to comparisons between orthogroups for each of the seven classes with two named paralogs (e.g., Wnt2/Wnt2b, Wnt3/Wnt3a, Wnt5a/Wnt5b, etc.).}
...
\end{center}
\caption{Two-axes histograms displaying the correlation between BLASTP bit score and sequence lengths among mammalian WNT proteins (1,367,031 comparisons for the 1,654 sequences included in this study).
...
It is perfectly feasible, however, to include an arbitrary number of sequence properties into the similarity metric, so we have devised a new scoring method that forces a global alignment of all sequences, then considers both a substitution matrix and predicted secondary structure properties.
Figure~\ref{fig:rdmclscores} illustrates that this new scoring function is independent of sequence length and, while correlated with the Emms transform scores, presents a unique dynamic range of values for MCL\@.
The details of the new RD-MCL scoring function are elaborated in Materials and Methods section~\ref{subsec:seq-sim-func} and is further compared against BLAST-based metrics for orthogroup assignment in section~\ref{subsec:TuningParams}.


\begin{figure}[t]
  \begin{center}
  \includegraphics[height=0.6\textheight]{../figures/rdmcl_scoring.eps}
\end{center}
...
The granularity of MCL is controlled by two parameters \textemdash{} the inflation coefficient and the edge similarity threshold~\cite{VanDongen:kJZ890qx} \textemdash{} and modulating these parameters will change the size and number of clusters returned.
Local maxima in cluster number and orthogroup score may arise within this parameter space (Figure X), meaning a simple hill-climbing optimization algorithm is not appropriate.
Instead, RD-MCL implements Metropolis coupled Markov chain Monte Carlo~\cite{Hastings:1970iw} to search for optimal MCL parameters, using our orthogroup score as the fitness function.
It should be noted that the edge similarity threshold has a stronger influence over MCL than the inflation coefficient, which makes intuitive sense because this parameter directly affects the information content in the graph by pruning away edges.
This increased sparsity will isolate clusters from one another even before the similarity matrix is transformed with MCL\@.


Note that OrthoFinder also uses a static inflation/edge similarity threshold for MCL~\cite{Emms:2015ig}
